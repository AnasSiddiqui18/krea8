...
export const sandbox = {
    ...
    getFile: async (filePath: string, sbxId: string) => {
        try {
            const file = await axios.get(`/sandbox/file/${sbxId}?filePath=${filePath}`)

            const parsed = getFilesFromSandboxSchema.safeParse(file.data)

            if (!parsed.success)
                return (console.log(z.treeifyError(parsed.error)), sendError("Invalid response from /sandbox/file"))
    ...
    updateFile: async (filePath: string, sbxId: string, content: string) => {
        try {
            const response = await axios.patch(`/sandbox/file/${sbxId}?filePath=${filePath}`, { content })
