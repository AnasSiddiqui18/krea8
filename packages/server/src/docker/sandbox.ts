...
export class Sandbox {
    ...
    async getOrPullImage(imageName: string, sbxId: string) {
        try {
            console.log("pull image fn runs")

            const image = this.docker.getImage(imageName)
            await image.inspect()

            console.log("spinning container")
    ...
    async spinContainer(imageName: string, sbxId: string) {
        ...
        try {
            ...
            const container = await this.docker.createContainer({
                ...
                HostConfig: {
                    Binds: [`/home/anas/projects/krea8/krea8/packages/server/sandboxes/sandbox-${sbxId}:/app`],
                    PortBindings: {
            ...
            this.activeContainers.set(sbxId, {
                ...
                isServerReady: true,
                hasError: true,
                errorMessage: String("message" in error ? error?.message : "Failed to spin sandbox"),
            })
        }
    }
}
